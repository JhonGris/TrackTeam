...
function parseRamDetalle(ramDetalle: string | null | undefined): RamModule[] {
  if (!ramDetalle) return [{ id: '1', marca: '', capacidadGb: '', velocidad: '', slot: '' }]
  try {
    const parsed = JSON.parse(ramDetalle)
    if (Array.isArray(parsed) && parsed.length > 0) {
      return parsed.map((ram: any, index: number) => ({
        id: (index + 1).toString(),
        marca: ram.marca || '',
    ...
    console.error('Error parsing ramDetalle:', e)
  }
  return [{ id: '1', marca: '', capacidadGb: '', velocidad: '', slot: '' }]
}

function parseDiscosDetalle(discosDetalle: string | null | undefined): DiscoDuro[] {
  if (!discosDetalle) return [{ id: '1', marca: '', serial: '', tipo: '', capacidadGb: '' }]
  try {
    const parsed = JSON.parse(discosDetalle)
    if (Array.isArray(parsed) && parsed.length > 0) {
      return parsed.map((disco: any, index: number) => ({
        id: (index + 1).toString(),
        marca: disco.marca || '',
        serial: disco.serial || '',
        tipo: disco.tipo || '',
    ...
    console.error('Error parsing discosDetalle:', e)
  }
  return [{ id: '1', marca: '', serial: '', tipo: '', capacidadGb: '' }]
}

function parseGpuDetalle(gpuDetalle: string | null | undefined): GpuInfo {
  if (!gpuDetalle) return { marca: '', modelo: '', serial: '', memoriaGb: '' }
  try {
    const parsed = JSON.parse(gpuDetalle)
    return {
      marca: parsed.marca || '',
      modelo: parsed.modelo || '',
      serial: parsed.serial || '',
      memoriaGb: parsed.memoriaGb?.toString() || ''
    }
  } catch (e) {
    console.error('Error parsing gpuDetalle:', e)
  }
  return { marca: '', modelo: '', serial: '', memoriaGb: '' }
}
...
export function EditarEquipoDialog({ equipo, open, onOpenChange }: EditarEquipoDialogProps) {
  ...
  const [selectedColaboradorId, setSelectedColaboradorId] = useState<string>(equipo.colaboradorId || 'sin-asignar')
  
  // Form fields (controlled for selects)
  const [tipo, setTipo] = useState<string>(equipo.tipo || 'Desktop')
  const [marca, setMarca] = useState<string>(equipo.marca || '')
  const [estadoSalud, setEstadoSalud] = useState<string>(equipo.estadoSalud || 'Bueno')
  const [estado, setEstado] = useState<string>(equipo.estado || 'Activo')
  
  // RAM modules (dynamic)
  const [ramModules, setRamModules] = useState<RamModule[]>(() => {
    // If we have ram but no ramDetalle, create a single module with the total ram
    if (equipo.ram && !equipo.ramDetalle) {
      return [{ id: '1', marca: '', capacidadGb: equipo.ram.toString(), velocidad: '', slot: '' }]
    }
    return parseRamDetalle(equipo.ramDetalle)
  })
  
  // Discos duros (dynamic)
  const [discosDuros, setDiscosDuros] = useState<DiscoDuro[]>(() => {
    // If we have almacenamiento but no discosDetalle, parse it
    if (equipo.almacenamiento && !equipo.discosDetalle) {
      const match = equipo.almacenamiento.match(/(\d+)GB?\s*(SSD|HDD|NVMe)?/i)
      if (match) {
        return [{
          id: '1',
          marca: '',
    ...
    return parseDiscosDetalle(equipo.discosDetalle)
  })
  
  // GPU
  const [gpu, setGpu] = useState<GpuInfo>(() => {
    if (equipo.gpuDetalle) {
      return parseGpuDetalle(equipo.gpuDetalle)
    }
    // Try to parse from gpu string
    if (equipo.gpu && equipo.gpu !== 'Integrada') {
      const parts = equipo.gpu.split(' ')
      return {
        marca: parts[0] || '',
  ...
  useEffect(() => {
    setSelectedColaboradorId(equipo.colaboradorId || 'sin-asignar')
    setTipo(equipo.tipo || 'Desktop')
    setMarca(equipo.marca || '')
    setEstadoSalud(equipo.estadoSalud || 'Bueno')
    setEstado(equipo.estado || 'Activo')
    
    // Reset RAM
    if (equipo.ram && !equipo.ramDetalle) {
      setRamModules([{ id: '1', marca: '', capacidadGb: equipo.ram.toString(), velocidad: '', slot: '' }])
    } else {
      setRamModules(parseRamDetalle(equipo.ramDetalle))
    }
    ...
    if (equipo.almacenamiento && !equipo.discosDetalle) {
      ...
      if (match) {
        ...
        setDiscosDuros(parseDiscosDetalle(equipo.discosDetalle))
      }
    } else {
      setDiscosDuros(parseDiscosDetalle(equipo.discosDetalle))
    }
    ...
    // Reset GPU
    if (equipo.gpuDetalle) {
      setGpu(parseGpuDetalle(equipo.gpuDetalle))
    } else if (equipo.gpu && equipo.gpu !== 'Integrada') {
      const parts = equipo.gpu.split(' ')
      setGpu({
        marca: parts[0] || '',
        modelo: parts.slice(1).join(' ') || '',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form action={formAction} className="space-y-6">
          ...
          <input type="hidden" name="ram" value={ramTotal || equipo.ram || 8} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="date"
                  defaultValue={equipo.fechaAdquisicion ? new Date(equipo.fechaAdquisicion).toISOString().split('T')[0] : ''}
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  name="fechaGarantia"
                  type="date"
                  defaultValue={equipo.fechaGarantia ? new Date(equipo.fechaGarantia).toISOString().split('T')[0] : ''}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="number"
                  min="0"
                  defaultValue={equipo.pantallas || 1}
                  placeholder="1, 2, 3..."
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  defaultValue={equipo.resolucionPantalla || ''}
                  placeholder="1920x1080, 4K..."
                />
              </div>

              <div className="flex items-center space-x-2 pt-6">
                <Checkbox id="tieneTeclado" name="tieneTeclado" defaultChecked={equipo.tieneTeclado} />
              ...
              <div className="flex items-center space-x-2 pt-6">
                <Checkbox id="tieneMouse" name="tieneMouse" defaultChecked={equipo.tieneMouse} />
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Input
                  ...
                  defaultValue={equipo.otrosPeriferico || ''}
